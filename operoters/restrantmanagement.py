...
def Ref():
    ...
    sql = f"insert into restaurent values ({rand.get()}, {Fries.get()}, {Largefries.get()},{Burger.get()},{Filet.get()},{Subtotal.get()},{Total.get()},{Service_Charge.get()},{Drinks.get()},{Tax.get()},{cost.get()},{Cheese_burger.get()})"
    print(sql)
    cur.execute(sql)
    cur.close()
    conn.commit()
