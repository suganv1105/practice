...
def addstdrec(usn, name, mobileno, address, email):
    ...
    cur.execute("""INSERT INTO student VALUES (:usn,:name,:mobileno,:address,:email)""",
                {'usn': usn, 'name': name, 'mobileno': mobileno, 'address': address, 'email': email})
...
def updatestddata(usn, name, mobileno, address, email):
    con = sqlite3.connect("student.db")
    cur = con.cursor()
    cur.execute("UPDATE student SET name=:name,mobileno=:mobileno,address=:address,email=:email WHERE usn=:usn",
                {'name': name, 'mobileno': mobileno, 'address': address, 'email': email, 'usn': usn})
    con.commit()
    con.close()
...
def addmoderator(idm, namem, password, emailm, contactno):
    con = sqlite3.connect("student.db")
    cur = con.cursor()
    cur.execute("""INSERT INTO moderator VALUES (:usr,:namem,:password,:emailm,:contactno)""",
...
def updaterecmoderator(idm, namem, password, emailm, contactno):
    ...
    cur = con.cursor()
    cur.execute(
        "UPDATE moderator SET namem=:namem,password=:password,emailm=:emailm,contactno=:contactno WHERE idm=:idm",
...
def addfeedetails(usn, amount, status, payment_date, penalty):
    con = sqlite3.connect("student.db")
    cur = con.cursor()
    cur.execute(""" INSERT INTO fee VALUES(:usn,:amount,:status,:payment_date,:penalty) """,
                {'usn': usn, 'amount': amount, 'status': status, 'payment_date': payment_date, 'penalty': penalty})
    con.commit()
    con.close()
...
def updatefeerec(usn, amount, status, date, penalty):
    con = sqlite3.connect("student.db")
    cur = con.cursor()
    cur.execute("UPDATE fee SET amount=:amount,status=:status,date=:date,penalty=:penalty WHERE usn=:usn",
                {'amount': amount, 'status': status, 'date': date, 'penalty': penalty, 'usn': usn})
    con.commit()
    con.close()
...
def addsubject(usn, sub1, sub2, sub3, backlogs):
    con = sqlite3.connect("student.db")
    cur = con.cursor()
    cur.execute("""INSERT INTO subject VALUES (:usn,:sub1,:sub2,:sub3,:backlogs)""",
                {'usn': usn, 'sub1': sub1, 'sub2': sub2, 'sub3': sub3, 'backlogs': backlogs})
    con.commit()
    con.close()
...
def updaterecsubject(usn, sub1, sub2, sub3, backlogs):
    con = sqlite3.connect("student.db")
    cur = con.cursor()
    cur.execute("UPDATE subject SET sub1=:sub1,sub2=:sub2,sub3=:sub3,backlogs=:backlogs WHERE usn=:usn",
                {'sub1': sub1, 'sub2': sub2, 'sub3': sub3, 'backlogs': backlogs, 'usn': usn})
    con.commit()
    con.close()
...
def addmark(usn, iat1, iat2, iat3, ex, avg, total):
    con = sqlite3.connect("student.db")
    cur = con.cursor()
    cur.execute("""INSERT INTO performance VALUES(:usn,:iat1,:iat2,:iat3,:AVG,:external,:total)""",
                {'usn': usn, 'iat1': iat1, 'iat2': iat2, 'iat3': iat3, 'external': ex, 'AVG': avg, 'total': total})
    con.commit()
    con.close()
